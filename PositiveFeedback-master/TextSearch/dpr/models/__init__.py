...
def init_wrapped_biencoder(args, **kwargs):
    if importlib.util.find_spec("transformers") is None:
        raise RuntimeError("Please install transformers lib")
    from .hf_models import get_wrapped_biencoder_components

    return get_wrapped_biencoder_components(args, **kwargs)
...
def init_wrapped_tenzorizer(args, **kwargs):
    if importlib.util.find_spec("transformers") is None:
        raise RuntimeError("Please install transformers lib")
    from .hf_models import get_wrapped_tensorizer

    return get_wrapped_tensorizer(args)
...
BIENCODER_INITIALIZERS = {
    ...
    "large_emb_models": init_wrapped_biencoder,
    # "uae_large_v1": init_wrapped_biencoder
}
...
TENSORIZER_INITIALIZERS = {
    ...
    "large_emb_models": init_wrapped_tenzorizer,
    # "uae_large_v1": init_wrapped_tenzorizer
}
