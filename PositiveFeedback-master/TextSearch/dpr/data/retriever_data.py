...
import jsonlines
import torch
from dpr.data.biencoder_data import (BiEncoderPassage, get_dpr_files,
                                     normalize_passage, read_nq_tables_jsonl,
...
class CsvQASrc(QASrc):
    ...
    def load_data(self, capacity=-1):
        ...
        with open(self.file) as ifile:
            ...
            if ~capacity:
                pbar = tqdm(reader, ncols=100, desc='read-csv', total=capacity)
            else:
                pbar = tqdm(reader, ncols=100, desc='read-csv')
            for row in pbar:
                ...
                if ~capacity and len(data) >= capacity:
                    break
...
class CsvCtxSrc(RetrieverData):
    ...
    def load_data_to(self, ctxs: Dict[object, BiEncoderPassage], capacity=-1):
        super().load_data()
        ...
        with open(self.file) as ifile:
            ...
            for row in pbar:
                ...
                counter += 1
                if ~capacity and counter >= capacity:
                    break
            print(
                f"{self.file} sample_id: {sample_id} Prefix: {self.id_prefix} id_col: {row[self.id_col]}"
            )
            print(f"{self.file} counter: {counter}")
