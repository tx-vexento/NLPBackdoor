...
def get_global_dense_features(data_dir, global_dense_feature_list, label_dict):
    ...
    if global_dense_feature_list != "none":
        print("Using global dense vector features = %s" % global_dense_feature_list)
...
def limit_dataset_size(dataset, limit_examples):
    ...
    if limit_examples:
        print("Limiting dataset to {:d} examples".format(limit_examples))
...
def limit_styles(dataset, specific_style_train, split, reverse_label_dict):
    ...
    if split in ["train", "test"] and -1 not in specific_style_train:
        print("Preserving authors = {}".format(", ".join([reverse_label_dict[x] for x in specific_style_train])))
        dataset = [x for x in dataset if x["suffix_style"] in specific_style_train]
        print("Remaining instances after author filtering = {:d} / {:d}".format(len(dataset), original_dataset_size))
    return dataset
...
def update_config(args, config):
    if args.global_dense_feature_list != "none":
        ...
        print("Using {:d} dense feature vectors.".format(global_dense_length))
    else:
