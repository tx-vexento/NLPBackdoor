...
class ParseNet(nn.Module):
    ...
    def encode_batch(self, inputs, lengths):
        ...
        all_hids, (enc_last_hid, _) = self.encoder(
    ...
    def decode_step(
        ...
        trans_weights = self.compute_transformation_attention(
            prev_hid, trans_embs, trans_lens
        )
    ...
    def batch_beam_search(
        ...
        while True:
            ...
            if num_examples != trim_hids.size(0):
                ...
                curr_out = (
                    trim_hids.unsqueeze(1)
                    .expand(d1, rep_factor, d2, d3)
                    .contiguous()
                    .view(-1, d2, d3)
                )
                curr_out_lens = (
                    out_trimmed_lens.unsqueeze(1)
                    .expand(d1, rep_factor)
                    .contiguous()
                    .view(-1)
                )
            else:
            ...
            hn, cn, attn_weights, attn_ctx, p_copy = self.decode_step(
                nsteps,
                prev_words,
                hs,
                cs,
            ...
            final_preds = (
                p_copy.unsqueeze(1) * vocab_scores + (1 - p_copy.unsqueeze(1)) * preds
            )
                ...
                if num_examples == len(beam_dict):
                    ...
                    for z in range(beam_size):
                        ...
                        beam_candidates.append(
                            (preds[word_idx].item(), ex_hn, ex_cn, [word_idx])
                        )
                    ...
                    for o_idx, ob in enumerate(origin_beams):
                        ...
                        for z in range(beam_size):
                            ...
                            beam_candidates.append(
                                (
                                    curr_prob + float(preds[word_idx].cpu().item()),
                                    curr_hn,
                                    curr_cn,
                                    seq + [word_idx],
                                )
            ...
            nsteps += 1
            if nsteps > max_steps:
                # print(f"reached max steps: {nsteps}")
                break
        return beam_dict
...
class SCPN(nn.Module):
    def __init__(
        self, d_word, d_hid, d_nt, d_trans, len_voc, len_trans_voc, use_input_parse
    ):
        ...
        if use_input_parse:
            ...
            self.encoder = nn.LSTM(
                d_word, d_hid, num_layers=1, bidirectional=True, batch_first=True
            )
    ...
    def encode_batch(self, inputs, trans, lengths):
        ...
        if self.use_input_parse:
            in_embs = torch.cat(
                [in_embs, trans.unsqueeze(1).expand(bsz, max_len, self.d_trans)], dim=2
            )
        ...
        all_hids = self.encoder_proj(all_hids.view(-1, self.d_hid * 2)).view(
            bsz, max_len, self.d_hid
        )
    ...
    def encode_transformations(self, trans, lengths, return_last=True):
        ...
        all_hids, (enc_last_hid, _) = self.trans_encoder(
            pack(in_embs[indices], lens.tolist(), batch_first=True),
    ...
    def decode_step(
        ...
        # concatenate w/ transformation embeddings
        trans_weights = self.compute_transformation_attention(
            prev_hid[1], trans_embs, trans_lens
        )
        trans_ctx = torch.sum(trans_weights.unsqueeze(2) * trans_embs, dim=1)
        decoder_input = torch.cat([word_input, trans_ctx.unsqueeze(1)], dim=2)
    ...
    def batch_beam_search(
        ...
        out_trans_hids = self.encode_transformations(
        ...
        while True:
            ...
            if num_examples != out_trans_hids.size(0):
                ...
                rep_factor = num_examples // d1
                curr_out = (
                    out_trans_hids.unsqueeze(1)
                    .expand(d1, rep_factor, d2, d3)
                    .contiguous()
                    .view(-1, d2, d3)
                )
                curr_out_lens = (
                    out_trans_lens.unsqueeze(1)
                    .expand(d1, rep_factor)
                    .contiguous()
                    .view(-1)
                )
